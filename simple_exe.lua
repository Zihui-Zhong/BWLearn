...
while total_battles < 40 do
    ...
    while not tc.state.game_ended do
		...
		for i, uid in ipairs(uids) do
			...
			if thisUnit == nil then
				table.insert(units,0)
				table.insert(units,0)
				for j, uid in ipairs(uids) do 
				...
				for j, second_pass_uid in ipairs(uids) do
					...
					if thatUnit == nil then
						...
						local x = thatUnit.position[1] - thisUnits.position[1]
						local y = thatUnit.position[2] - thisUnits.position[2]
						local dist = x*x + y*y
